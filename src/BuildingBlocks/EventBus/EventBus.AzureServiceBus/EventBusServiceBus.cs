...
namespace EventBus.AzureServiceBus
{
    public class EventBusServiceBus : BaseEventBus
    {
        ...
        {
            return new SubscriptionClient(base.EventBusConfig.EventBusConnectionString, base.EventBusConfig.DefaultTopicName, base.GetSubName(eventnName));
        }
        ...
        private ISubscriptionClient CreateSubscriptionClientIfNotExist(string eventName)
        {
            ...
            var exist = this._managementClient.SubscriptionExistsAsync(base.EventBusConfig.DefaultTopicName, base.GetSubName(eventName)).GetAwaiter().GetResult();

            if (!exist)
            {
                this._managementClient.CreateSubscriptionAsync(base.EventBusConfig.DefaultTopicName, base.GetSubName(eventName)).GetAwaiter().GetResult();
        ...
        private void CreateRuleIfNotExist(string eventName, ISubscriptionClient subscriptionClient)
        {
            ...
            try
            {
                var rule = this._managementClient.GetRuleAsync(base.EventBusConfig.DefaultTopicName, base.GetSubName(eventName), eventName).GetAwaiter().GetResult();
                ruleExist = rule != null;
            }
            catch (MessagingEntityNotFoundException)
            {
